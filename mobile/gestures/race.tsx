import { Skia, SkiaMutableValue, SkiaValue, SkMatrix, SkRect, useSharedValueEffect } from "@shopify/react-native-skia";
import {
  Gesture,
  GestureDetector,
  PanGestureHandler,
...
export function ActionsHandler({ dimensions, matrix }: ActionsHandlerProps) {
  const mat = useSharedValue(identity4);
  const start = useSharedValue({ x: 0, y: 0 });
  const offset = useSharedValue({ x: 0, y: 0 });
  const { x, y, width, height } = dimensions;

  // todo: use the picture matrix to manipulate the image!
